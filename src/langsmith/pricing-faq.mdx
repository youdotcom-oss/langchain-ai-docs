...
You may choose to sign up in any supported LangSmith Cloud region, including GCP US, GCP EU, GCP APAC, and AWS US. See the [cloud architecture reference](/langsmith/cloud#cloud-architecture-and-scalability) for more details. If you're on the Enterprise plan, we can deliver LangSmith to run on your kubernetes cluster in AWS, GCP, or Azure so that data never leaves your environment.
